...
public class UserService implements IUserService {
    ...
    public boolean createUser(@NotNull UserReq userReq) {
        ...
        if (userReq != null && userReq.email() != null && userReq.username() != null) {
            ...
            eventPublisher.publishEvent(new EntityChangedEvent(user, TypeOfChangesEnum.CREATED_BY_USER.getValue()));
            return true;
        }
        return false;
    }
    ...
    public boolean updateUser(@NotNull UserReq userReq) {
        if (!userReq.username().isEmpty() && !userReq.email().isEmpty()) {
            ...
            if (userForUpdate.isPresent()) {
                ...
                addUserAndSaveToFile(user);
                eventPublisher.publishEvent(new EntityChangedEvent(user, TypeOfChangesEnum.UPDATE_OF_DATA.getValue()));
                return true;
            } else {
                return false;
            }
    ...
    public boolean deleteUserById(@NotNull String userIdString) {
        try {
            ...
            if (userToRemove.isPresent()) {
                ...
                eventPublisher.publishEvent(new EntityChangedEvent(userToRemove, TypeOfChangesEnum.MANUAL_DELETED.getValue()));
                return true;
            }
        } catch (IllegalArgumentException e) {
            LOGGER.info("Invalid UUID format: " + userIdString);
